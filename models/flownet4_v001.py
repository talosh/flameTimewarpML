...
class Model:
    ...
    def __init__(self, status = dict(), torch = None):
        ...
        class Flownet(Module):
            ...
            def forward(self, img0, img1, f0, f1, timestep, mask, flow, scale=1):
                ...
                if isinstance(timestep, torch.Tensor) and timestep.dim() == 1:
                    # per-sample ratios (N,) -> (N, 1, 1, 1) so the broadcast
                    # below multiplies per batch item, not per channel
        ...
        class FlownetCas(Module):
            # return_levels=True  (training model): the result carries all four
            # pyramid levels so the loss can supervise every scale (deep
            # supervision). return_levels=False (inference model): only the
            # finest level is materialized, no intermediate clones.
            def __init__(self, return_levels=True):
                super().__init__()
            ...
            def forward(self, img0, img1, timestep=0.5, scale=[12, 8, 4, 1], iterations=1, gt=None):
                ...
                merged[3] = warp(img0, flow[:, :2]) * mask + warp(img1, flow[:, 2:4]) * (1 - mask)
        ...
        class FlownetCasInfer(FlownetCas):
            def __init__(self):
                super().__init__(return_levels=False)

        self.model = FlownetCasInfer
