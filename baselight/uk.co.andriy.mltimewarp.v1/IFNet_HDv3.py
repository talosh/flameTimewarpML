...
class IFNet(nn.Module):
    ...
    def forward(self, x, scale_list=[4,2,1], scale=1.0):
        ...
        warped_img0 = warp(x[:, :3], F1_large[:, :2], device=self.device)
        warped_img1 = warp(x[:, 3:], F1_large[:, 2:4], device=self.device)
        ...
        warped_img0 = warp(x[:, :3], F2_large[:, :2], device=self.device)
        warped_img1 = warp(x[:, 3:], F2_large[:, 2:4], device=self.device)
