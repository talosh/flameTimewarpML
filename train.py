...
def main():
    ...
    while True:
        for batch_idx in range(len(dataset)):
            ...
            loss += criterion_mse(output_d2, target) * (1 / 16 ** 2)
            loss += criterion_mse(output_d3, target) * (1 / 8 ** 2)
            loss += criterion_mse(output_d4, target) * (1 / 4 ** 2)
            loss += criterion_mse(output_d5, target) * (1 / 2 ** 2)
            loss += loss_cons
