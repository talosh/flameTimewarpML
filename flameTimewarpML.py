    ...
    def flownet412(self, img0, img1, ratio, model_path):
        ...
        class IFNet(nn.Module):
            ...
            def forward(self, img0, img1, timestep=0.5, scale_list=[8, 4, 2, 1], training=False, fastmode=True, ensemble=True):
                ...
                # '''
                raft_img0 = F.interpolate(img0 * 2 - 1, scale_factor = 1 / 1, mode="bilinear", align_corners=False)
                raft_img1 = F.interpolate(img1 * 2 - 1, scale_factor = 1 / 1, mode="bilinear", align_corners=False)
                ...
                raft_flow_f = F.interpolate(raft_flow_f, scale_factor = 1, mode="bilinear", align_corners=False, recompute_scale_factor=False) * 1 * timestep
                raft_flow_b = F.interpolate(raft_flow_b, scale_factor = 1, mode="bilinear", align_corners=False, recompute_scale_factor=False) * 1 * (1 - timestep)
