    ...
    def __init__(self, status = dict(), torch = None):
        ...
        class FlownetDeepDualHead(Module):
            def __init__(self, in_planes, c=64):
                ...
                self.mix1f = DownMixToSpat(c, c)
                self.mix2 = UpMix(c, cd)
                self.mix2f = DownMixToSpat(c, c)
                self.mix3 = Mix(c, cd)
                self.mix3f = DownMixToSpat(c, c)
                self.revmix1 = DownMix(c, cd)
                self.revmix1f = UpMixToFreq(c, c)
                self.revmix2 = DownMix(c, cd)
                self.revmix2f = UpMixToFreq(c, c)
            ...
            def forward(self, img0, img1, f0, f1, scale=1):
                ...
                xf = imgs - imgs.mean(dim=(0, 2, 3), keepdim=True)
        ...
        class FlownetCas(Module):
            def __init__(self):
                super().__init__()
                self.block0 = FlownetDeepDualHead(6+16+2, c=64) # images + feat + timestep + lingrid
