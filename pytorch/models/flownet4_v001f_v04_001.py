    ...
    def __init__(self, status = dict(), torch = None):
        ...
        class FlownetCas(Module):
            def __init__(self):
                ...
                self.block1 = FlownetDeepDualHead(6+3+20+1+1+1+2+1+4, 12+20+8+1, c=128) # FlownetDeepDualHead(9+30+1+1+4+1+2, 22+30+1, c=128) # images + feat + timestep + lingrid + mask + conf + flow
                self.block2 = FlownetDeepDualHead(6+3+20+1+1+1+2+1+4, 12+20+8+1, c=96) # FlownetLT(6+2+1+1+1, c=48) # None # FlownetDeepDualHead(9+30+1+1+4+1+2, 22+30+1, c=112) # images + feat + timestep + lingrid + mask + conf + flow
