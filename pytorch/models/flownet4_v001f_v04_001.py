...
class Model:
    ...
    def __init__(self, status = dict(), torch = None):
        ...
        class FlownetLT(Module):
            ...
            def forward(self, img0, img1, timestep, mask, conf, scale=1):
                n, c, h, w = img0.shape
                sh, sw = round(h * (1 / scale)), round(w * (1 / scale))
                ph = self.maxdepth - (sh % self.maxdepth)
                pw = self.maxdepth - (sw % self.maxdepth)
        ...
        class FlownetCas(Module):
            def __init__(self):
                ...
                self.block3 = FlownetLT(6+2+1+1+1, c=48) # None # FlownetDeepDualHead(9+30+1+1+4+1+2, 22+30+1, c=112) # images + feat + timestep + lingrid + mask + conf + flow
                self.encode = Head()
            ...
            def forward(self, img0, img1, timestep=0.5, scale=[16, 8, 4, 1], iterations=1, gt=None):
                ...
                flow_d, mask_d, conf_d = self.block3(img0, img1, timestep, mask, conf, scale=scale[3])

                flow_list[3] = flow + flow_d
                conf_list[3] = torch.sigmoid(conf + conf_d) #
                mask_list[3] = torch.sigmoid(mask + mask_d) #
