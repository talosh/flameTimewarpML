...
class Model:
    ...
    info = {
        'name': 'Stabnet4_v001fe_v04_001',
        'file': 'stabnet4_v001fe_v04_001.py',
        'ratio_support': True
    }
    ...
    def __init__(self, status = dict(), torch = None):
        ...
        class HeadF(Module):
            def __init__(self):
                super().__init__()
                self.cnn0f = torch.nn.Conv2d(12, 48, 3, 2, 1)
            ...
            def forward(self, x):
                x = x * 2 - 1
                x = x - x.mean()
                xf = self.cnn0f(torch.cat(to_freq(x), to_freq_mph(x)), 1)
                xf = self.relu1(xf)
                xf = self.cnn1f(xf)
                xf = self.relu2(xf)
                xf = self.cnn2f(xf)
        ...
        class UpMixToFreq(Module):
            def __init__(self, c, cd):
                super().__init__()
                self.conv = torch.nn.ConvTranspose2d(cd*4, c, 4, 2, 1)
                self.beta = torch.nn.Parameter(torch.ones((1, c, 1, 1)), requires_grad=True)
        ...
        class FlownetDeepDualHead(Module):
            def __init__(self, in_planes, in_planes_fx, c=64):
                ...
                self.conv0f = conv(in_planes_fx*2, c, 3, 2, 1)
            ...
            def forward(self, img0, img1, scale=1):
                ...
                feat = self.conv0f(torch.cat((to_freq(imgs_scaled), to_freq_mph(imgs_scaled)), 1))
