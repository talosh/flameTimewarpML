...
# Increased filters and output filters in Head
...
class Model:
    def __init__(self, status = dict(), torch = None):
        ...
        class FlownetCas(Module):
            def __init__(self):
                super().__init__()
                self.block0 = Flownet(7+24, c=224)
                self.block1 = Flownet(8+4+24, c=192)
                self.block2 = Flownet(8+4+24, c=128)
                self.block3 = Flownet(8+4+24, c=96)
                self.encode = Head()
