...
class Model:
    ...
    info = {
        'name': 'Flownet4_v001ea',
        'file': 'flownet4_v001ea.py',
        'ratio_support': True
    }
    ...
    def __init__(self, status = dict(), torch = None):
        ...
        class CauchyActivation(Module):
            def __init__(self):
                super().__init__()
                # Initializing λ1, λ2, d as trainable parameters
                self.lambda1 = torch.nn.Parameter(torch.tensor(1.0), requires_grad=True)
                self.lambda2 = torch.nn.Parameter(torch.tensor(1.0), requires_grad=True)
                self.d = torch.nn.Parameter(torch.tensor(1.0), requires_grad=True)
            def forward(self, x):
                x2_d2 = x ** 2 + self.d ** 2
        ...
        class FlownetCas(Module):
            ...
            def forward(self, img0, img1, timestep=0.5, scale=[16, 8, 4, 1], iterations=1, gt=None):
                ...
                flow_list[3] = flow
                conf_list[3] = torch.sigmoid(conf) # compress(conf) # 
                mask_list[3] = torch.sigmoid(mask) # compress(mask) # 
