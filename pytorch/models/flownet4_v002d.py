...
# feeding back conf layer as state layer
# changed conf / state layer to 3
    ...
    def __init__(self, status = dict(), torch = None):
        ...
        class Flownet(Module):
            def __init__(self, in_planes, c=64):
                ...
                self.lastconv = torch.nn.Sequential(
                    torch.nn.ConvTranspose2d(c, 4*9, 4, 2, 1),
            ...
            def forward(self, img0, img1, f0, f1, timestep, mask, conf, flow, scale=1):
                ...
                if flow is not None:
                    ...
                    conf = torch.nn.functional.interpolate(conf, scale_factor= 1. / scale, mode="bilinear", align_corners=False)
        ...
        class FlownetCas(Module):
            def __init__(self):
                ...
                self.block1 = Flownet(8+4+16+4, c=128)
                self.block2 = Flownet(8+4+16+4, c=96)
                self.block3 = Flownet(8+4+16+4, c=64)
                self.encode = Head()
