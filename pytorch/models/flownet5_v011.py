...
class Model:
    ...
    def __init__(self, status = dict(), torch = None):
        ...
        class Flownet2LT(Module):
            ...
            def forward(self, img0, img1, f0, f1, timestep, mask, conf, flow, state, scale=1):
                n, c, h, w = img0.shape
        ...
        class Flownet2(Module):
            def __init__(self, in_planes, c=64):
                ...
                self.lastconv = torch.nn.Sequential(
                    torch.nn.ConvTranspose2d(c, 10, 4, 2, 1),
                )
                self.maxdepth = 4
        ...
        class Flownet4(Module):
            def __init__(self, in_planes, c=96):
                super().__init__()
                self.conv0 = torch.nn.Sequential(
                    torch.nn.Conv2d(in_planes, max(c//2, in_planes*2), 5, 2, 2, padding_mode = 'zeros'),
                    torch.nn.PReLU(max(c//2, in_planes*2), 0.2),
                    torch.nn.Conv2d(max(c//2, in_planes*2), c, 5, 2, 2, padding_mode = 'reflect'),
                    torch.nn.PReLU(c, 0.2),
                    )
                self.convblock = torch.nn.Sequential(
                    ResConvAtt(c),
                ...
                self.lastconv = torch.nn.Sequential(
                    torch.nn.ConvTranspose2d(c, 4*10, 4, 2, 1),
        ...
        class FlownetCas(Module):
            ...
            def forward(self, img0, img1, timestep=0.5, scale=[12, 8, 4, 1], iterations=4, gt=None):
                ...
                flow, mask, conf, state = self.block1(
                    img0, 
                    img1, 
                    f0, 
                    f1, 
                ...
                flow, mask, conf, state = self.block2(
                    img0, 
                    img1, 
                    f0, 
                    f1, 
                ...
                flow_d, mask_d, conf_d, state = self.block5(
                    img0,
                    img1,
                    f0,
                    f1,
                ...
                flow_list[5] = flow
                conf_list[5] = torch.sigmoid(conf) #
                mask_list[5] = torch.sigmoid(mask) #
                # merged[3] = warp(img0, flow[:, :2]) * mask_list[3] + warp(img1, flow[:, 2:4]) * (1 - mask_list[3])

                flow_d, mask_d, conf_d, state = self.block6(
                    img0,
                    img1,
                    f0,
                    f1,
                ...
                flow_list[6] = flow
                conf_list[6] = torch.sigmoid(conf) #
                mask_list[6] = torch.sigmoid(mask) #
                # merged[3] = warp(img0, flow[:, :2]) * mask_list[3] + warp(img1, flow[:, 2:4]) * (1 - mask_list[3])

                flow_d, mask_d, conf_d, state = self.block7(
                    img0,
                    img1,
                    f0,
                    f1,
                ...
                flow_list[7] = flow
                conf_list[7] = torch.sigmoid(conf) #
                mask_list[7] = torch.sigmoid(mask) #
                # merged[3] = warp(img0, flow[:, :2]) * mask_list[3] + warp(img1, flow[:, 2:4]) * (1 - mask_list[3])

                flow_d, mask_d, conf_d = self.block8(
                    img0,
                    img1,
                    f0,
                    f1,
                ...
                flow_list[8] = flow
                conf_list[8] = torch.sigmoid(conf) #
