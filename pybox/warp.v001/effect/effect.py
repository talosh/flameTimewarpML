        ...
        if msg.get('data') == 'process':
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    res_inv = res_inv.squeeze(0).permute(1, 2, 0).cpu()

                    write_image_file(result_file0, np.ascontiguousarray(res_fwd.numpy(force=True)), img['spec'])
                    write_image_file(result_file1, np.ascontiguousarray(res_inv.numpy(force=True)), img['spec'])
